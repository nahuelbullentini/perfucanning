...
const SAMPLE_PRODUCTS = [
  { id: "pf-001", sku: "PF-001", stock: 12, price: 24500, description: "Eau de Parfum floral amaderado, 100ml", status: "activo", image: "https://images.unsplash.com/photo-1541643600914-78b084683601?w=500&q=80" },
  { id: "pf-002", sku: "PF-002", stock: 5, price: 31900, description: "Colonia cítrica fresca, 90ml", status: "activo", image: "https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?w=500&q=80" },
  { id: "pf-003", sku: "PF-003", stock: 0, price: 18700, description: "Eau de Toilette oriental especiado, 75ml", status: "activo", image: "https://images.unsplash.com/photo-1523293182086-7651a899d37f?w=500&q=80" },
  { id: "pf-004", sku: "PF-004", stock: 20, price: 15300, description: "Body splash vainilla y almizcle, 200ml", status: "activo", image: "https://images.unsplash.com/photo-1615634260167-c8cdede054de?w=500&q=80" },
];
...
app.post("/api/admin/import-excel", requireAdmin, (req, res) => {
  try {
    ...
    if (importedBySku.size === 0) {
      return res.status(400).json({ error: "No se encontraron filas válidas en el Excel" });
    }

    const seenSkus = new Set();

    // Productos que ya existían: se actualizan si están en el Excel de hoy,
    // o pasan a "suspendido" (sin borrarse) si no aparecieron.
    const updatedExisting = products.map((p) => {
      const imported = importedBySku.get(p.sku);
      if (imported) {
        seenSkus.add(p.sku);
        return { ...p, ...imported, status: "activo" };
      }
      return { ...p, status: "suspendido" };
    });

    // Filas del Excel que son productos nuevos (SKU no existía antes).
    const newOnes = Array.from(importedBySku.values())
      .filter((imp) => !seenSkus.has(imp.sku))
      .map((imp) => ({
        id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
        image: null,
        status: "activo",
    ...
    products = [...updatedExisting, ...newOnes];
    writeJSON(PRODUCTS_PATH, products);
    res.json({ ok: true, count: products.length, suspendidos: products.filter((p) => p.status === "suspendido").length });
  } catch (err) {
